...
function getRectangleString(width, height) {
  ...
  const heightLeft = '│';
  const enter = '\n';
  let rectangle = topLeft + '─'.repeat(width - 2) + topRight + enter;
  for (let i = 0; i < height - 2; i += 1) {
    rectangle += heightLeft + ' '.repeat(width - 2) + heightLeft + enter;
  }
  rectangle += bottomLeft + '─'.repeat(width - 2) + bottomRight + enter;
  return rectangle;
}
